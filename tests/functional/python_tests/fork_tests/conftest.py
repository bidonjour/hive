...
def prepared_networks() -> Dict:
    ...
    tt.logger.info('Preparing fixture world_with_witnesses')
    ...
    beta_api_node = tt.ApiNode(network=beta_net)
    ...
    yield {  # TODO: Consider using this_dict['Alpha'].node('ApiNode0')
        'Alpha': {
            'Network': alpha_net,
            'ApiNode': alpha_api_node,
        },
        'Beta': {
            'Network': beta_net,
            'ApiNode': beta_api_node,
        },
    }
